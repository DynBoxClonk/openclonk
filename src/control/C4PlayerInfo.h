...
public:
	...
	enum Flags
	{
		...
		PIF_DoEliminationCheck = 1 << 13, // eliminate player if crew is empty
		...
		PIF_SyncFlags = PIF_Joined | PIF_Removed | PIF_HasRes | PIF_InScenarioFile | PIF_JoinedForSavegameOnly | PIF_Disconnected | PIF_Won | PIF_VotedOut | PIF_AttributesFixed | PIF_NoScenarioInit | PIF_DoEliminationCheck | PIF_Invisible | PIF_NoScenarioSave,

		// flags to be copied from savegame-player for takeover
		PIF_SavegameTakeoverFlags = PIF_Joined | PIF_Removed | PIF_JoinIssued | PIF_AttributesFixed | PIF_NoScenarioInit | PIF_DoEliminationCheck | PIF_Invisible | PIF_NoScenarioSave,
	};
...
public:
	...
	bool HasEliminationCheck() const { return !!(dwFlags & PIF_DoEliminationCheck); }
	void SetEliminationCheck(bool active);
